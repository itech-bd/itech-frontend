...
export default async function StudentCourseDetailPage({ params }: { params: Promise<{ locale: string; course: string }> }) {
  ...
  return (
    <main className="space-y-5">
      ...
      <section className="grid gap-5 xl:grid-cols-[.9fr_1.1fr] xl:items-start">
      ...
      <StudentCard className="flex h-[34rem] min-h-0 flex-col overflow-hidden sm:h-[36rem]">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <h2 className="mt-3 text-2xl font-black text-[color:var(--text-heading)]">Learning path details</h2>
          </div>
          <div className="inline-flex w-fit items-center gap-2 rounded-full border border-[color:var(--border-default)] bg-[color:var(--surface-secondary)] px-3 py-1.5 text-xs font-black text-[color:var(--text-muted)]">
            <Layers3 aria-hidden className="h-4 w-4" />
            Scroll to read
          </div>
        </div>
        {data.course.description ? (
          <div
            className="site-prose mt-5 min-h-0 flex-1 basis-0 overflow-y-scroll overscroll-contain rounded-[1.25rem] border border-[color:var(--border-default)]/70 bg-[color:var(--surface-secondary)]/70 p-5 pr-3 text-sm leading-7 [scrollbar-gutter:stable] [scrollbar-width:thin] [scrollbar-color:var(--border-default)_transparent]"
            dangerouslySetInnerHTML={{ __html: sanitizeCmsHtml(data.course.description) }}
          />
        ) : (
          <p className="mt-5 min-h-0 flex-1 basis-0 overflow-y-scroll rounded-[1.25rem] border border-[color:var(--border-default)]/70 bg-[color:var(--surface-secondary)]/70 p-5 text-sm leading-7 text-[color:var(--text-body)] [scrollbar-gutter:stable] [scrollbar-width:thin] [scrollbar-color:var(--border-default)_transparent]">
            Course details will appear here when they are added.
          </p>
        )}
      </StudentCard>
    </main>
